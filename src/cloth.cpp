...
void Cloth::simulate(double frames_per_sec, double simulation_steps, ClothParameters *cp,
  ...
  for (auto &pm : point_masses) {
    if (pm.pinned) continue; // Skip pinned points
    
    Vector3D acceleration = pm.forces / mass;
    
    // Calculate new position using Verlet integration
    Vector3D new_position = pm.position + (1 - cp->damping) * (pm.position - pm.last_position) + acceleration * delta_t * delta_t;

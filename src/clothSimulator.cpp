...
void ClothSimulator::load_textures() {
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB32F, m_cloth_tex_width, m_cloth_tex_height, 
               0, GL_RGB, GL_FLOAT, nullptr);
  
  std::cout << "Initialized cloth particles texture with size: " << m_cloth_tex_width << "x" << m_cloth_tex_height << std::endl;
...
void ClothSimulator::drawContents() {
  ...
  if (cloth && cloth->point_masses.size() > 0) {
    ...
    glActiveTexture(GL_TEXTURE6);
    glBindTexture(GL_TEXTURE_2D, m_cloth_particles_tex);
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, m_cloth_tex_width, m_cloth_tex_height,
                   GL_RGB, GL_FLOAT, particle_positions.data());
  }
